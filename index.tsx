...
function View() {
  ...
  async function importBackup() {
    ...
    try {
      const parsed = parseInterchangeBackup(JSON.parse(FileManager.readAsStringSync(paths[0])))
      const importedFolders = interchangeFoldersToPaths(parsed)
      const importedFavorites: FavoriteItem[] = parsed.favorites.map((favorite) => ({
        id: favorite.id || makeRowId(), name: favorite.name, texts: favorite.texts,
      ...
      for (const item of importedFavorites) {
        ...
        if (sameName) {
          ...
          if (choice === 1) nextFavorites.splice(nextFavorites.indexOf(sameName), 1)
        }
      ...
      await alert(`导入完成：新增 ${nextFavorites.length - favorites.length} 条收藏`)
    } catch (error) {
