...
// ============ 条码类型定义 ============
// 支持的条码类型：QR Code / Code 128-B / Code 39 / EAN-13 / EAN-8 / UPC-A / ITF-14 / Codabar
export type BarcodeType = "qr" | "code128" | "code39" | "ean13" | "ean8" | "upca" | "itf14" | "codabar"
...
export const BARCODE_TYPES: BarcodeTypeInfo[] = [
  { id: "qr", name: "QR Code", hint: "任意文本，最多 200 个字符" },
  { id: "code128", name: "Code 128-B", hint: "任意文本，最多 80 个字符" },
  { id: "code39", name: "Code 39", hint: "A-Z 0-9 - . 空格 $ / + %，最多 40 个字符" },
  { id: "ean13", name: "EAN-13", hint: "12~13 位数字" },
  { id: "ean8", name: "EAN-8", hint: "7~8 位数字" },
  { id: "upca", name: "UPC-A", hint: "11~12 位数字" },
  { id: "itf14", name: "ITF-14", hint: "13~14 位数字" },
  { id: "codabar", name: "Codabar", hint: "数字及 - $ : / . +，首尾 A-D，最多 40 个字符" },
]

// 为避免一维条码过窄、图片过大及分享失败，对可变长度格式设置上限。
export const MAX_TEXT_LENGTHS: Record<BarcodeType, number> = {
  qr: 200,
  code128: 80,
  code39: 40,
  ean13: 13,
...
export const MAX_BARCODE_ITEMS = 100

export function maxTextLength(type: BarcodeType): number {
  return MAX_TEXT_LENGTHS[type]
}
...
// ============ EAN-8 / EAN-13 / UPC-A ============
// EAN-8 使用 4 位左侧数据 + 4 位右侧数据，最后一位为校验位
...
// 编码 EAN-8：接受 7 位并自动计算校验位；接受 8 位时验证最后一位校验位
export function encodeEAN8(text: string): number[] | null {
  const cleaned = text.replace(/\s/g, "")
  if (!/^\d{7,8}$/.test(cleaned)) return null
  const data = cleaned.slice(0, 7)
  const checkDigit = ean8CheckDigit(data)
  if (cleaned.length === 8 && Number(cleaned[7]) !== checkDigit) return null
  const full = data + String(checkDigit)
  const bits: number[] = []
  const push = (s: string) => { for (const ch of s) bits.push(ch === "1" ? 1 : 0) }
  push("101")
  for (let i = 0; i < 4; i++) push(EAN_L[Number(full[i])])
  push("01010")
  for (let i = 4; i < 8; i++) push(EAN_R[Number(full[i])])
  push("101")
  return bits
}

// EAN-13 / UPC-A 使用的 L/G/R 模式
const EAN_L = [
...
// 编码 EAN-13：接受 12 位并自动计算校验位；接受 13 位时验证最后一位校验位
export function encodeEAN13(text: string): number[] | null {
  const cleaned = text.replace(/\s/g, "")
  if (!/^\d{12,13}$/.test(cleaned)) return null
  const data = cleaned.slice(0, 12)
  const checkDigit = ean13CheckDigit(data)
  if (cleaned.length === 13 && Number(cleaned[12]) !== checkDigit) return null
  const full = data + String(checkDigit)
...
// 编码 UPC-A：接受 11 位并自动计算校验位；接受 12 位时验证最后一位校验位
export function encodeUPCA(text: string): number[] | null {
  const cleaned = text.replace(/\s/g, "")
  if (!/^\d{11,12}$/.test(cleaned)) return null
  const data = cleaned.slice(0, 11)
  const checkDigit = upcaCheckDigit(data)
  if (cleaned.length === 12 && Number(cleaned[11]) !== checkDigit) return null
  const full = data + String(checkDigit)
  const bits: number[] = []
  const push = (s: string) => {
    for (const ch of s) bits.push(ch === "1" ? 1 : 0)
...
// 编码 ITF-14：接受 13 位并自动计算校验位；接受 14 位时验证最后一位校验位
export function encodeITF14(text: string): number[] | null {
  const cleaned = text.replace(/\s/g, "")
  if (!/^\d{13,14}$/.test(cleaned)) return null
  const data = cleaned.slice(0, 13)
  const checkDigit = itf14CheckDigit(data)
  if (cleaned.length === 14 && Number(cleaned[13]) !== checkDigit) return null
  const full = data + String(checkDigit)
  const bits: number[] = []
  // 起始符：窄条 窄空 窄条 窄空
  bits.push(1, 0, 1, 0)
...
// ============ Codabar ============
// Codabar 每个字符由 7 个条/空元素组成，1 表示宽元素，0 表示窄元素。
const CODABAR_PATTERNS: Record<string, string> = {
  ...
  ":": "1001010", "/": "1010010", ".": "1010100", "+": "0010110",
  "A": "0011010", "B": "0101001", "C": "1001011", "D": "1101000",
}

function codabarCharModules(pattern: string): number[] {
  const modules: number[] = []
  for (let i = 0; i < pattern.length; i++) {
    const width = pattern[i] === "1" ? 2 : 1
    const isBar = i % 2 === 0
    for (let j = 0; j < width; j++) modules.push(isBar ? 1 : 0)
  }
  return modules
}

export function encodeCodabar(text: string): number[] | null {
  let value = text.toUpperCase().replace(/\s/g, "")
  if (!value) return null
  if (!/^[A-D]/.test(value)) value = `A${value}B`
  if (!/^[A-D][0-9$+:/\.\-]+[A-D]$/.test(value)) return null
  const bits: number[] = []
  for (let i = 0; i < value.length; i++) {
    const pattern = CODABAR_PATTERNS[value[i]]
    if (!pattern) return null
    if (i > 0) bits.push(0)
    bits.push(...codabarCharModules(pattern))
  }
  return bits
}
...
export function encodeBarcode(type: BarcodeType, text: string): number[] | null {
  switch (type) {
    ...
    case "codabar": return encodeCodabar(text)
